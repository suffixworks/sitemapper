...
import { SUFFIX_DOMAIN } from "@/auth.config";
    ...
    <main className="flex min-h-[100dvh] items-center justify-center bg-[#EDF0F5] p-6">
      <div className="w-full max-w-[360px] rounded-2xl border border-[#E1E6EF] bg-white px-8 py-9 text-center shadow-[0_8px_30px_rgba(20,30,60,.12)]">
        ...
        <p className="mx-auto mt-2 max-w-[280px] text-[13.5px] leading-relaxed text-[#7A8496]">
          Sign in with your{" "}
          <span className="font-medium text-[#1B2130]">@{SUFFIX_DOMAIN}</span> Google
          account to build and review sitemaps.
        </p>

        {error && (
          <p className="mt-4 rounded-lg bg-[#FDECEA] px-3 py-2 text-[12.5px] text-[#B23B37]">
            {error === "AccessDenied"
              ? `Only @${SUFFIX_DOMAIN} accounts can sign in.`
        ...
        <form action={googleSignIn}>
          <button
            type="submit"
            className="mt-6 flex h-11 w-full items-center justify-center gap-2.5 rounded-[10px] border border-[#E1E6EF] bg-white text-[14px] font-semibold text-[#1B2130] transition-colors hover:bg-[#F7F8FB]"
          >
            <GoogleMark />
